...
sudo apt update -qq
sudo apt install -y conky-all playerctl
...
# Copy configs
echo "[3/5] Installing Conky configs..."
mkdir -p "$CONKY_DIR"
...
echo "[4/5] Detecting network interface..."
...
Exec=sh -c "sleep 5 && conky -d -c $CONKY_DIR/rock-roll.conf && conky -d -c $CONKY_DIR/quotes.conf && conky -d -c $CONKY_DIR/nowplaying.conf && conky -d -c $CONKY_DIR/sysmon.conf"
Hidden=false
NoDisplay=false
...
echo "All 4 widgets are running!"
echo ""
